...
- Config at `~/.claude/garryscount/config.json` with `reset_hour` (default 5), `count_mode` ("default" or "yc-mode"), and `show_directory` (true/false, default false — shows current directory in orange on the status line)
